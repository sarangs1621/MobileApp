...
| 2 | Med | ~~**CSP is report-only**, not enforcing (`next.config.ts`).~~ **RESOLVED 2026-07-15**: enforced via per-request nonce + `'strict-dynamic'` in `apps/web/middleware.ts` (Next stamps the nonce on its inline scripts from the request-header CSP; `'unsafe-inline'` remains only as the legacy-browser fallback that nonce-aware browsers ignore). Verified: every `<script>` on served pages carries the nonce. | — |
