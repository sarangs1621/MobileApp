...
- **"Unique when present" → partial unique index** (raw SQL in the migration; `@@unique` can't express `WHERE`): `ReportCard ... WHERE "examId" IS NOT NULL` (ADR-009), `GuardianStudent("studentId") WHERE "isPrimary"`. When a column should dedupe, prefer a **non-null sentinel** so `@@unique` works (`Attendance.period @default(0)`).
...
- **Calendar-date columns** (attendance `date`, exam dates, leave from/to) should use **`@db.Date`** so the date is DB-typed and the unique key/range logic can't drift by a UTC off-by-one. (Recommended in the architecture review; confirm before the M2/M3 migrations.)
